                ...
                "problems": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "math_expression": {
                                "type": "string",
                                "description": "Valid mathematical expression suitable for safe evaluation with SymPy, e.g., (2 + 3) - 1/2 * 1/2"
                            }
                        },
...
# Safe evaluation function using SymPy
def safe_eval(expr):
    try:
        return float(sympify(expr).evalf())  # Converting to float for consistent output format
    except Exception as e:
        return f"Error: {e}"

# Calculate math expressions safely using SymPy
def calculate_math_expressions(problems):
    results = []
    for problem in problems:
        expression = problem["math_expression"]
        results.append(safe_eval(expression))
    return results
...
async def run_conversation(user_message: str):
    ...
    while cur_iter < MAX_ITER:
        ...
        if function_name == "calculator_tool":
            ...
            function_response = f"Ask the user to search with the following queries: {queries} and give you the search results."
